...
However, if one has the J2 jumper present but boots via another method (perhaps another slot in a multi-pak interface), then the ROM is never accessed and the Q signal will continue to be asserted to CART\*. This will prevent proper use of interrupts. If one is not using the autostart feature, remove the J2 jumper to prevent this contention.
